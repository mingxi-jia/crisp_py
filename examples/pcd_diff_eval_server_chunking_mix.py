...
# from collections import deque  # Not needed - using list instead
...
class RealTimeChunkingController:
    ...
    horizon = 16
    action_exec_size = 8 # Number of actions to use per chunk
    merge_range = 4 # Number of overlapping actions between chunks
    debug_output_dir = Path("debug_plots")  # Directory for debug plots

    assert action_exec_size + policy_delay <= horizon, "action_exec_size + policy_delay must be <= horizon"
    assert merge_range <= action_exec_size / 2, "merge_range must be < action_exec_size"
    ...
    def __init__(self, robot, gripper, obs_manager, joint_state_subscriber,
        ...
        self.action_queue = []  # Using list instead of deque for flexible indexing
        ...
        if self.debug_plotting:
            ...
            self.action_queue_snapshots = []  # List of (inference_idx, n_steps_done_at_snapshot, action_queue_copy)
            self.actual_poses = []  # List of (n_steps_done, x, y, z, yaw)
    ...
    def _need_inference(self):
        ...
        if len(self.action_queue) <= self.policy_delay + self.merge_range:
            return True
        return False
    ...
    def _run_inference(self):
        ...
        # Add actions to queue with merging
        if len(self.action_queue) == 0:
            ...
            # Merge and append new actions
            for action_idx, action in enumerate(actions[self.policy_delay: self.policy_delay + self.action_exec_size]):
                if action_idx < self.merge_range:
                    prior_weight = (self.merge_range - action_idx) / self.merge_range 
                    prior_weight = prior_weight / 2
                    action_merged = prior_weight * self.action_queue[-(action_idx+1)] + (1 - prior_weight) * action.copy()
                    self.action_queue[-(action_idx+1)] = action_merged
                else:
                    self.action_queue.append(action.copy())

        # Store action queue snapshot after merging for debug plotting
        if self.debug_plotting:
            # We need to track n_steps_done, but we don't have it here - will get it from run() method
            # For now, store with inference_count as a proxy
            self.action_queue_snapshots.append((self.inference_count, [a.copy() for a in self.action_queue]))
    ...
    def _plot_debug_results(self):
        """Generate plot showing merged action queue snapshots vs actual execution."""
        print("\nGenerating merged actions plot...")

        if len(self.action_queue_snapshots) == 0:
            print("No action queue snapshots to plot")
            return

        # Create colormap for different inference snapshots
        ...
        for ax, label in zip(axes.flatten(), labels):
            # Plot each action queue snapshot
            for snap_idx, (inf_idx, action_queue) in enumerate(self.action_queue_snapshots):
                # Calculate starting n_step for this snapshot
                # First snapshot starts at 0, subsequent snapshots start based on action_exec_size - merge_range
                if snap_idx == 0:
                    start_step = 0
                else:
                    start_step = (self.action_exec_size - self.merge_range) * snap_idx
                ...
                # Plot the merged queue snapshot
        ...
        # Save plot
        filename = self.debug_output_dir / "merged_action_queue.png"
        ...
        for ax, label in zip(axes.flatten(), labels):
            ...
            for inf_idx, (chunk_id, actions) in enumerate(self.all_predicted_chunks):
                ...
                if inf_idx == 0:
                    ...
                    start_step = (self.action_exec_size - self.merge_range) * inf_idx
