...
Gating mechanism: Only records when action norm > 0.003 or grasping action occurs.
No FPS throttling - records as fast as possible when conditions are met.
"""
...
class RecorderConfig:
    ...
    queue_size: int = 20
    sync_slop: float = 0.05  # 50ms max time difference for sync
    ...
    # Action gating threshold
    action_threshold: float = 0.003
...
class FrameRecorderNode(Node):
    def __init__(self):
        super().__init__('frame_recorder_node_fast')
        ...
        self.data_lock = threading.Lock()  # For gripper, eef_pose, joint_state, action
        ...
        # Current action state (continuously updated by teleop message)
        self.current_dx = 0.0
        self.current_dy = 0.0
        self.current_dz = 0.0
        self.current_droll = 0.0
        self.current_dpitch = 0.0
        ...
        self.get_logger().info(f"Action threshold: {self.config.action_threshold}")
    ...
    def _on_teleop_signal(self, msg: Int32MultiArray):
        ...
        This message continuously updates:
        1. Intervention state
        2. Current action state (dx, dy, dz, droll, dpitch, dyaw, gripper)
        """
        if len(msg.data) < 9:
            return
        ...
        dx, dy, dz = msg.data[1], msg.data[2], msg.data[3]
        droll, dpitch, dyaw = msg.data[4], msg.data[5], msg.data[6]
        gripper_action = msg.data[7]
        ...
        with self.data_lock:
            ...
            self.current_dy = dy
            self.current_dz = dz
            self.current_droll = droll
            self.current_dpitch = dpitch
            self.current_dyaw = dyaw
            self.current_gripper_action = gripper_action
    ...
    def _on_synchronized_images(self, *msgs):
        ...
        with self.data_lock:
            ...
            dy = self.current_dy
            dz = self.current_dz
            droll = self.current_droll
            dpitch = self.current_dpitch
            dyaw = self.current_dyaw
            gripper_action = self.current_gripper_action

        # Check gating condition based on current action state
        translation_norm = np.sqrt(dx**2 + dy**2 + dz**2)
        rotation_norm = np.sqrt(droll**2 + dpitch**2 + dyaw**2)

        should_record = (
            translation_norm > self.config.action_threshold or
            rotation_norm > self.config.action_threshold or
            abs(gripper_action) > 0
        )

        # Only record frames when gating condition is met
        if not should_record:
            return

        self.get_logger().info(
            f"Recording frame - trans: {translation_norm:.4f}, rot: {rotation_norm:.4f}, gripper: {gripper_action}"
        )

        try:
            self.frame_queue.put_nowait((msgs, gripper, eef_pose, joint_state, intervention_state))
        except queue.Full:
    ...
    def _writer_loop(self):
        ...
        while self.writer_running.is_set() or not self.frame_queue.empty():
            ...
            msgs, gripper, eef_pose, joint_state, intervention_state = bundle
