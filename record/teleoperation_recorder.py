...
class RecorderConfig:
    ...
    queue_size: int = 100
    sync_slop: float = 0.03  # 30ms max time difference for strict sync
...
class FrameRecorderNode(Node):
    ...
    def _on_teleop_signal(self, msg: Int32MultiArray):
        ...
        # Skip if reset is pressed (index 7)
        if msg.data[7]:
            return

        # Check for movement (indices 0-5) or gripper toggle (index 6)
        has_command = any(msg.data[i] != 0 for i in range(6)) or msg.data[6]
        if has_command:
            with self.data_lock:
    ...
    def _on_synchronized_images(self, *msgs):
        ...
        try:
            self.frame_queue.put_nowait((msgs, gripper, eef_pose, joint_state))
        except queue.Full:
    ...
    def _writer_loop(self):
        ...
        while self.writer_running.is_set() or not self.frame_queue.empty():
            ...
            msgs, gripper, eef_pose, joint_state = bundle
    ...
    def _save_state_data(self, state_path: str):
        """Save gripper states, EEF poses, and joint states."""
