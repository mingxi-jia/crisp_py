...
for time_step, hand_pose in enumerate(hand_poses):
    ...
    # Convert 10D action to Pose, then to gripper frame
    pose, grasp = ten_d_action_to_pose(action, clip=False)
    gripper_position, gripper_rotation = convert_action_from_fingertip_to_gripper(pose)

    target_pose.position = gripper_position
